...
function aplicarRestricoesDeStatus(status) {
    ...
    if (status === 'banido') {
        ...
        const bannerHtml = `<div id="${bannerID}" class="fixed top-0 left-0 right-0 z-[60] bg-yellow-500 text-white text-xs font-bold px-4 py-2 text-center shadow-xl flex justify-between items-center animate-slideDown"><span class="flex items-center gap-2"><i class="animate-pulse">⚠️</i> CONTA SUSPENSA - Recursos Limitados</span><button onclick="window.abrirChatSuporte()" class="bg-black/20 px-3 py-1 rounded hover:bg-black/30">Resolver</button></div>`;
        body.insertAdjacentHTML('beforeend', bannerHtml);
        document.getElementById('header-main')?.classList.add('mt-8');
    }
    else if (status === 'rejeitado') {
        const bannerHtml = `<div id="${bannerID}" class="fixed top-0 left-0 right-0 z-[60] bg-red-500 text-white text-xs font-bold px-4 py-2 text-center shadow-xl flex justify-between items-center animate-slideDown"><span class="flex items-center gap-2">🚫 DADOS REJEITADOS - Atualize seu perfil</span><button onclick="window.abrirConfiguracaoServicos()" class="bg-white/20 px-3 py-1 rounded hover:bg-white/30">Editar</button></div>`;
        body.insertAdjacentHTML('beforeend', bannerHtml);
        document.getElementById('header-main')?.classList.add('mt-8');
    }
    else { document.getElementById('header-main')?.classList.remove('mt-8'); }
}
function removerBloqueiosVisuais() { document.getElementById("bloqueio-total-overlay")?.remove(); document.getElementById("status-alert-banner")?.remove(); }
