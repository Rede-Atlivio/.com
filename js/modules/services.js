...
let meusServicos = []; 
let basePriceAtual = 0; // Para negociação

// --- 1. FUNÇÕES DE NAVEGAÇÃO (PRIORIDADE ALTA) ---
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        ...
        renderizarFiltros(); 
        renderizarCabecalhoUsuario(); // Isso corrige a foto "Carregando..."
        
        try { carregarPrestadoresOnline(); } catch(e) { console.error("Index erro:", e); }
...
window.validarOferta = (valorInput) => {
    ...
    const min = basePriceAtual * 0.80; // -20%
    const max = basePriceAtual * 1.30; // +30%
...
window.enviarPropostaAgora = async () => { 
    ...
    if(!targetId) return alert("Erro: Prestador não identificado."); 
...
// --- 4. PRESTADOR & CLIENTE (RENDERIZAÇÃO) ---
