...
// --- ONLINE/OFFLINE ---
function configurarBotaoOnline() { const toggle = document.getElementById('online-toggle'); if(!toggle) return; toggle.checked = false; toggle.addEventListener('change', async (e) => { if (e.target.checked) { const userDoc = await getDoc(doc(db, "usuarios", auth.currentUser.uid)); const userData = userDoc.data(); if (userData.setup_profissional_ok && userData.services_offered && userData.services_offered.length > 0) { meusServicos = userData.services_offered; document.getElementById('setup-name').value = userData.nome_profissional || ""; renderMyServicesList(); await ficarOnline(); } else { document.getElementById('provider-setup-modal').classList.remove('hidden'); e.target.checked = false; } } else { await ficarOffline(); } }); }
async function ficarOnline() { if (!auth.currentUser) return; if (navigator.geolocation) { navigator.geolocation.getCurrentPosition(async (position) => { const userDoc = await getDoc(doc(db, "usuarios", auth.currentUser.uid)); const userData = userDoc.data(); const categoriasAtivas = meusServicos.map(s => s.category); await setDoc(doc(db, "active_providers", auth.currentUser.uid), { uid: auth.currentUser.uid, email: auth.currentUser.email, lat: position.coords.latitude, lng: position.coords.longitude, tenant_id: 'atlivio_fsa_01', nome_profissional: userData.nome_profissional, foto_perfil: auth.currentUser.photoURL || null, services: meusServicos, categories: categoriasAtivas, last_seen: serverTimestamp() }); }, (error) => { alert("Erro de GPS: " + error.message); document.getElementById('online-toggle').checked = false; }); } else { alert("Seu navegador não suporta GPS."); } }
...
function escutarMeusChamados() {
    ...
    if(!containerAtivos || !containerHistorico) { setTimeout(escutarMeusChamados, 1000); return; }
...
function carregarPrestadoresOnline(forcar = false) { const listaContainer = document.getElementById('lista-prestadores-realtime'); if(!listaContainer) { setTimeout(carregarPrestadoresOnline, 1000); return; } if(listenerPrestadoresAtivo && !forcar) return; if(forcar) listaContainer.innerHTML = ""; let q = query(collection(db, "active_providers")); if (categoriaAtiva !== 'Todos') { q = query(collection(db, "active_providers"), where("categories", "array-contains", categoriaAtiva)); } onSnapshot(q, (snap) => { listenerPrestadoresAtivo = true; listaContainer.innerHTML = ""; if (snap.empty) { renderEmptyState(listaContainer); } else { snap.forEach(d => { const p = d.data(); if(auth.currentUser && p.uid === auth.currentUser.uid) return; if (!p.services || p.services.length === 0) return; let servicoExibido = null; if (categoriaAtiva !== 'Todos') { servicoExibido = p.services.find(s => s.category === categoriaAtiva); } else { servicoExibido = p.services[0]; } if(!servicoExibido) return; const nomeExibicao = p.nome_profissional || "Prestador"; const fotoUrl = p.foto_perfil; const avatarContent = fotoUrl ? `<img src="${fotoUrl}" class="w-full h-full object-cover rounded-full">` : `<div class="w-full h-full bg-blue-50 text-blue-500 flex items-center justify-center text-xl font-bold rounded-full">${nomeExibicao.charAt(0)}</div>`; listaContainer.innerHTML += `<div class="bg-white p-4 rounded-xl border border-blue-100 shadow-sm flex flex-col items-center relative group hover:shadow-md transition cursor-pointer" onclick="abrirPerfilPublico('${p.uid}')"><div class="absolute top-2 right-2 w-2 h-2 bg-green-500 rounded-full animate-pulse"></div><div class="w-12 h-12 mb-2 border border-blue-100 rounded-full">${avatarContent}</div><span class="bg-blue-100 text-blue-800 text-[9px] font-black px-2 py-0.5 rounded-full uppercase mb-1">${servicoExibido.category}</span><h4 class="font-bold text-sm text-gray-800 text-center leading-tight mb-1 truncate w-full">${nomeExibicao}</h4><p class="text-[10px] text-gray-500 mb-3 font-bold">A partir de: R$ ${servicoExibido.price}</p><button onclick="event.stopPropagation(); abrirModalSolicitacao('${p.uid}', '${nomeExibicao}', '${servicoExibido.price}')" class="w-full bg-blue-600 text-white py-2 rounded-lg text-[10px] font-bold uppercase hover:bg-blue-700 transition shadow-sm">Solicitar</button></div>`; }); } }); }
...
window.abrirModalSolicitacao = (uid, nomePrestador, precoBase) => { 
    ...
    // --- CORREÇÃO: GRAVAR ID NO INPUT HIDDEN (DOM) ---
    // Isso garante que o ID não se perca se a variável global falhar
    const hiddenInput = document.getElementById('target-provider-id');
    if(hiddenInput) hiddenInput.value = uid;
    
    targetProviderEmail = nomePrestador; // Mantemos para exibir no chat (menos crítico)
    
    // --- CORREÇÃO: USAR VARIÁVEL GLOBAL WINDOW PARA O PREÇO ---
...
window.enviarPropostaAgora = async () => { 
    // --- CORREÇÃO: LER ID DO INPUT HIDDEN (DOM) ---
    const hiddenInput = document.getElementById('target-provider-id');
    const targetId = hiddenInput ? hiddenInput.value : null;

    if(!targetId) return alert("Erro crítico: Prestador não identificado. Tente recarregar.");
    ...
    try { 
        ...
        await addDoc(collection(db, "orders"), { 
            ...
            provider_email: targetProviderEmail || "Prestador", 
            service_date: data, 
            service_time: hora, 
