    ...
    if (isOnline) {
        container.innerHTML = `
            <div class="text-gray-400 mb-4 py-10 animate-fadeIn text-center">
                ...
                <p class="text-xs text-gray-400 mt-2">Sincronizado com o Banco de Dados.</p>
            </div>`;
    } else {
        container.innerHTML = `
            <div class="text-gray-400 mb-4 py-10 animate-fadeIn text-center">
                ...
                <p class="text-xs mt-2">Ative o botão "Trabalhar" para ficar visível.</p>
            </div>`;
    }
}
...
// --- 3. LÓGICA DE SERVIÇOS E FILTROS ---
...
function renderMyServicesList() {
    ...
    meusServicos.forEach((srv, index) => {
        list.innerHTML += `<div class="bg-gray-50 p-3 rounded mb-2 flex justify-between border border-gray-100">
            <span class="font-bold text-blue-900">${srv.category} - R$ ${srv.price}</span>
            <button onclick="window.removeServiceLocal(${index})" class="text-red-400 font-bold">&times;</button>
        </div>`;
    });
}
...
// --- 5. INICIALIZAÇÃO E SINCRONIA CRÍTICA ---
