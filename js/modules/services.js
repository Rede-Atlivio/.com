...
// ======================================================
// LÓGICA DE SUB-ABAS (CLIENTE E PRESTADOR) - BLINDADA
// ======================================================
...
// ======================================================
// PRESTADOR ONLINE (LÓGICA HÍBRIDA)
// ======================================================
...
// --- ESCUTAR CHAMADOS (DISTRIBUÍDO NAS ABAS DO PRESTADOR) ---
    ...
    onSnapshot(q, (snap) => {
        ...
        if(!snap.empty) {
            snap.forEach(d => {
                ...
                // === PEDIDOS ATIVOS (Aguardando Aceite ou Em Andamento) ===
                ...
                // === PEDIDOS HISTÓRICO ===
                    ...
                    <div class="bg-gray-100 p-3 rounded-xl border border-gray-200 opacity-70 mb-2">
                        <p class="font-bold text-xs text-gray-600">${pedido.status === 'completed' ? '✅ Serviço Finalizado' : '❌ Recusado'}</p>
        ...
        if(!hasAtivos) containerAtivos.innerHTML = `<p class="text-center text-gray-400 text-xs py-4">Nenhum pedido ativo.</p>`;
...
function renderizarCabecalhoUsuario() {
    ...
    if(toggle) { 
        ...
        if(!secPrestador.classList.contains('hidden') && btnEdit) btnEdit.classList.remove('hidden');
    }
}
...
function renderizarFiltros() {
    ...
    categoriasDisponiveis.forEach(cat => { html += `<button onclick="filtrarCategoria('${cat}')" class="filter-pill px-4 py-2 rounded-full border border-blue-100 bg-white text-gray-500 text-[10px] font-bold uppercase shadow-sm hover:bg-blue-50">${cat}</button>`; });
...
window.filtrarCategoria = (cat) => {
    ...
    btns.forEach(btn => {
        if(btn.innerText.toUpperCase() === cat.toUpperCase()) { btn.classList.add('active'); btn.classList.remove('text-gray-500'); } else { btn.classList.remove('active'); btn.classList.add('text-gray-500'); }
    });
...
function carregarPrestadoresOnline(forcar = false) {
    ...
    onSnapshot(q, (snap) => {
        listenerPrestadoresAtivo = true; listaContainer.innerHTML = ""; 
        if (snap.empty) { renderEmptyState(listaContainer); } else {
            snap.forEach(d => {
                const p = d.data();
                if(auth.currentUser && p.uid === auth.currentUser.uid) return;
                let servicoExibido = null;
                if (categoriaAtiva !== 'Todos') { servicoExibido = p.services.find(s => s.category === categoriaAtiva); } else { servicoExibido = p.services[0]; }
...
window.abrirPerfilPublico = async (uid) => {
    const modal = document.getElementById('provider-profile-modal'); const listaServicos = document.getElementById('public-services-list');
    modal.classList.remove('hidden'); listaServicos.innerHTML = '<p class="text-center text-xs">Carregando...</p>';
    try {
        ...
        p.services.forEach(s => {
            listaServicos.innerHTML += `<div class="bg-gray-50 p-3 rounded-lg border border-gray-100 flex justify-between items-center"><div class="flex-1"><span class="block font-bold text-xs text-blue-900 uppercase">${s.category}</span><span class="text-[10px] text-gray-500 block leading-tight">${s.description}</span></div><div class="text-right ml-2"><span class="block font-black text-sm text-gray-800">R$ ${s.price}</span><button onclick="document.getElementById('provider-profile-modal').classList.add('hidden'); abrirModalSolicitacao('${p.uid}', '${p.nome_profissional}', '${s.price}')" class="text-[9px] text-blue-600 font-bold border border-blue-200 px-2 py-1 rounded mt-1 hover:bg-blue-50">Contratar</button></div></div>`;
        });
    } catch (e) { console.error(e); listaServicos.innerHTML = '<p class="text-red-500 text-xs">Erro ao carregar perfil.</p>'; }
};
...
function escutarMeusPedidos() {
    if(!auth.currentUser) return;
    const containerAndamento = document.getElementById('meus-pedidos-andamento'); const containerHistorico = document.getElementById('meus-pedidos-historico');
    if(!containerAndamento || !containerHistorico) return;
    const q = query(collection(db, "orders"), where("client_id", "==", auth.currentUser.uid));
    onSnapshot(q, (snap) => {
        containerAndamento.innerHTML = ""; containerHistorico.innerHTML = ""; let hasAndamento = false; let hasHistorico = false;
        if(!snap.empty) {
            snap.forEach(d => {
                const pedido = d.data(); const nomePrestadorHistorico = pedido.provider_email || "Prestador"; 
                if (pedido.status === 'pending_acceptance' || pedido.status === 'reserved') {
                    hasAndamento = true;
