...
window.abrirConfiguracaoServicos = async () => {
    ...
    // --- LÓGICA DA TARJA (ATUALIZADA PARA STATUS INDIVIDUAL) ---
    // A tarja agora será gerada dentro do MAP para cada serviço individualmente
    // -----------------------

    const servicesHtml = s.length > 0 ? s.map((sv,i)=> {
        // Lógica de Status Individual
        const stSvc = sv.status || 'em_analise';
        let badgeSvc = "";
        
        if(stSvc === 'aprovado') {
            badgeSvc = `<span class="bg-green-100 text-green-700 border border-green-200 px-1.5 py-0.5 rounded text-[8px] font-black uppercase ml-2 tracking-wide">✅ APROVADO</span>`;
        } else if(stSvc === 'suspenso' || stSvc === 'reprovado') {
            badgeSvc = `<span class="bg-red-100 text-red-700 border border-red-200 px-1.5 py-0.5 rounded text-[8px] font-black uppercase ml-2 tracking-wide">🔴 SUSPENSO</span>`;
        } else {
            badgeSvc = `<span class="bg-yellow-100 text-yellow-700 border border-yellow-200 px-1.5 py-0.5 rounded text-[8px] font-black uppercase ml-2 tracking-wide">⏳ EM ANÁLISE</span>`;
        }
    ...
    }).join('') : '<p class="text-xs text-gray-400 italic text-center py-4 border border-dashed border-gray-300 rounded">Nenhum serviço adicionado.</p>';
...
window.addServiceLocal = async () => {
    ...
    let s = snap.exists() ? snap.data().services||[] : []; 
    
    // 🔥 CORREÇÃO PRINCIPAL: Injeta status individual
    s.push({
        category: c, 
        price: parseFloat(p), 
        description: d, 
        status: 'em_analise' // Nasce em análise, independente do perfil
    });
