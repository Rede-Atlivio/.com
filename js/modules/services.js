...
let clientIdParaAvaliar = null; // Para avaliação bilateral
...
function escutarMeusChamados() {
    ...
    // Variável para evitar som no primeiro load
        ...
        if (snap.empty) {
            ...
            snap.docChanges().forEach(change => {
                if (change.type === "added" && !isInitialLoad) {
                    ...
                    if(audio && change.doc.data().status === 'reserved') audio.play().catch(e => console.log("Som bloqueado pelo browser"));
                }
            });
            isInitialLoad = false;
            ...
            snap.forEach(d => {
                ...
                if (pedido.status === 'reserved') {
                    ...
                    // BOTÃO AVALIAR CLIENTE (Se ainda não avaliou)
                    const botaoAvaliar = !pedido.provider_reviewed 
                        ? `<button onclick="abrirModalAvaliacao('${d.id}', '${pedido.client_id}', 'cliente')" class="mt-2 w-full bg-blue-600 text-white py-2 rounded-lg font-bold text-[10px] uppercase">⭐ AVALIAR CLIENTE</button>` 
...
// --- ATUALIZAÇÃO V19.1: HISTÓRICO SEPARADO ---
    ...
    onSnapshot(q, (snap) => {
        ...
        snap.forEach(d => {
            ...
            // CARD HISTÓRICO
                ...
                temHistorico = true;
                const btnReview = !pedido.client_reviewed
                    ? `<button onclick="abrirModalAvaliacao('${d.id}', '${pedido.provider_id}', 'prestador')" class="w-full mt-2 bg-blue-100 text-blue-700 py-2 rounded-lg font-bold text-[10px] uppercase hover:bg-blue-200">⭐ AVALIAR PRESTADOR</button>`
                ...
                containerHistorico.innerHTML += `
                    <div class="bg-gray-50 p-3 rounded-xl border border-gray-200 mb-2">
                        <div class="flex justify-between items-center">
                            <span class="text-[10px] font-bold text-gray-500">Finalizado em ${new Date(pedido.completed_at?.seconds * 1000).toLocaleDateString()}</span>
                            <span class="text-[9px] bg-gray-200 text-gray-600 px-2 py-0.5 rounded">R$ ${pedido.service_value}</span>
                        </div>
                        <p class="text-xs font-bold text-gray-700 mt-1">${pedido.provider_email}</p>
                        ${btnReview}
                    </div>`;
                
                // POPUP AUTOMÁTICO SE NÃO AVALIOU
                if(!pedido.client_reviewed) {
                    setTimeout(() => abrirModalAvaliacao(d.id, pedido.provider_id, 'prestador'), 1000);
                }
            }
        });
...
window.abrirModalAvaliacao = (orderId, targetId, tipo) => {
    ...
    // Define quem estamos avaliando
    if(tipo === 'prestador') {
        providerIdParaAvaliar = targetId;
        clientIdParaAvaliar = null;
        document.getElementById('review-title').innerText = "Avaliar Prestador";
    } else {
        clientIdParaAvaliar = targetId;
        providerIdParaAvaliar = null;
        document.getElementById('review-title').innerText = "Avaliar Cliente";
    }
...
window.enviarAvaliacao = async () => {
    ...
    try {
        // Decide qual campo atualizar no pedido (client_reviewed ou provider_reviewed)
        const updateData = {};
        let targetUserId = "";

        if (providerIdParaAvaliar) {
            updateData.client_reviewed = true; // Cliente avaliou o prestador
            targetUserId = providerIdParaAvaliar;
        } else {
            updateData.provider_reviewed = true; // Prestador avaliou o cliente
            targetUserId = clientIdParaAvaliar;
        }

        // 1. Salva Avaliação
        await addDoc(collection(db, "reviews"), {
            order_id: orderIdParaAvaliar,
            target_user_id: targetUserId, // Quem recebeu a nota
            reviewer_id: auth.currentUser.uid, // Quem deu a nota
...
window.confirmarSolicitacao = async () => {
    ...
    try {
        ...
        await addDoc(collection(db, "orders"), {
            ...
            status: "reserved",
            chat_id: chatRoomId,
            client_reviewed: false, // Inits
            provider_reviewed: false, // Inits
            created_at: serverTimestamp()
        });
