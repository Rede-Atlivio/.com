...
export async function abrirChatPedido(orderId) {
    ...
    if (!painelChat || painelChat.parentElement !== document.body) {
        ...
        const stylePC = "width: 450px; height: 700px; max-height: 90vh; right: 20px; bottom: 20px; border-radius: 20px; border: 1px solid #e2e8f0; box-shadow: 0 20px 25px -5px rgb(0 0 0 / 0.1); display: flex; flex-direction: column; overflow: hidden; position: fixed;";
...
async function renderizarEstruturaChat(container, pedido, isProvider, orderId, step) {
    ...
    `<span class="text-[8px] font-bold text-amber-600 bg-amber-50 px-2 py-1 rounded border border-amber-200">🔒 CONTRATO ATIVO</span>`
           ...
           <div id="chat-messages" class="flex-1 flex flex-col bg-slate-50 relative overflow-hidden" style="height: 100%;">
               ...
               <div id="scroll-area-v16" class="custom-scrollbar p-4" style="flex: 1 1 0%; height: 0; overflow-y: auto; scroll-behavior: smooth; display: flex; flex-direction: column; background: #f8fafc;">
