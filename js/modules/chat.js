...
export async function abrirChatPedido(orderId) {
    ...
    if (!painelChat || painelChat.parentElement !== document.body) {
        ...
        const stylePC = "width: 450px; height: 85vh; right: 20px; bottom: 20px; border-radius: 20px; border: 1px solid #e2e8f0; box-shadow: 0 20px 25px -5px rgb(0 0 0 / 0.1); display: flex; flex-direction: column; overflow: hidden !important; position: fixed; contain: content;";
...
async function renderizarEstruturaChat(container, pedido, isProvider, orderId, step) {
    ...
    `<span class="text-[8px] font-bold text-amber-600 bg-amber-50 px-2 py-1 rounded border border-amber-200">🔒 CONTRATO ATIVO</span>`
           ...
           <div id="chat-messages" class="flex-1 bg-slate-50 relative" style="display: grid; grid-template-rows: auto 1fr auto; height: 100%; max-height: 100%; overflow: hidden !important;">
