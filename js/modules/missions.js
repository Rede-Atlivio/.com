...
// ⚖️ MOTOR DE AUDITORIA B2B (O CLIENTE JUIZ)
// Gil, esta função carrega as fotos que os usuários enviaram para este cliente específico
window.carregarAuditoriaB2B = async () => {
    const container = document.getElementById('sub-view-container');
    container.innerHTML = `
        <div class="py-20 text-center">
            <div class="loader mx-auto border-amber-500"></div>
            <p class="text-[9px] text-gray-500 font-black uppercase mt-4 animate-pulse tracking-widest">Buscando evidências coletadas...</p>
        </div>
    `;
    ...
    try {
        ...
        if (snap.empty) {
            container.innerHTML = `
                <div class="py-20 text-center border-2 border-dashed border-white/5 rounded-[40px] bg-slate-900/20">
                    <p class="text-gray-600 text-[10px] font-black uppercase italic tracking-tighter">Nenhum dado aguardando sua aprovação.</p>
                </div>
            `;
            return;
        }

        container.innerHTML = `<div class="grid grid-cols-1 gap-4 animate-fadeIn" id="lista-auditoria-cards"></div>`;
        const lista = document.getElementById('lista-auditoria-cards');

        snap.forEach(d => {
            const m = d.data();
            const gpsOk = m.gps_status === 'match';
            
            lista.innerHTML += `
                <div class="bg-slate-900 rounded-[32px] border border-white/10 p-5 space-y-4 shadow-2xl">
                    <div class="flex justify-between items-start">
                        <div>
                            <h4 class="text-white font-black text-xs uppercase tracking-tight">${m.mission_title}</h4>
                            <p class="text-[8px] text-gray-500 uppercase font-bold tracking-widest">Colaborador: ${m.user_name}</p>
                        </div>
                        <span class="px-2.5 py-1 rounded-full text-[7px] font-black uppercase ${gpsOk ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20 animate-pulse'}">
                            ${gpsOk ? '✅ GPS Confirmado' : '⚠️ Localização Suspeita'}
                        </span>
                    </div>

                    <div class="relative group cursor-pointer overflow-hidden rounded-2xl border border-white/5" onclick="window.visualizarProva('${m.proof_url}')">
                        <img src="${m.proof_url}" class="w-full h-52 object-cover group-hover:scale-105 transition-transform duration-500">
                        <div class="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                            <span class="bg-white text-black px-5 py-2 rounded-full text-[9px] font-black uppercase shadow-xl">Ampliar Foto</span>
                        </div>
                    </div>

                    <div class="flex gap-2">
                        <button onclick="window.resolverVereditoB2B('${d.id}', 'rejected')" class="flex-1 py-4 bg-red-600/10 text-red-500 rounded-2xl font-black text-[9px] uppercase border border-red-500/10 hover:bg-red-600 hover:text-white transition-all active:scale-95">Reprovar</button>
                        <button onclick="window.resolverVereditoB2B('${d.id}', 'approved')" class="flex-[2.5] py-4 bg-emerald-600 text-white rounded-2xl font-black text-[9px] uppercase shadow-lg shadow-emerald-900/20 hover:bg-emerald-500 transition-all active:scale-95">Aprovar e Pagar</button>
                    </div>
                </div>
            `;
        });
        ...
        console.error(e);
        container.innerHTML = `<p class="text-red-500 text-[10px] font-black text-center uppercase tracking-widest">Erro ao carregar auditoria.</p>`;
    }
};

// 🏛️ MOTOR DE VEREDITO B2B (ARBITRAGEM)
window.resolverVereditoB2B = async (docId, veredito) => {
    if (veredito === 'rejected') {
        if (!confirm("⚠️ REPROVAR ESTE DADO?\n\nA missão NÃO será excluída. Ela será enviada para arbitragem do suporte técnico (Gil) para verificação final.")) return;
        
        await updateDoc(doc(db, "mission_submissions", docId), { 
            status: 'disputed_by_b2b', 
            updated_at: serverTimestamp(),
            b2b_rejection_reason: 'Recusado pelo cliente no portal'
        });
        alert("👮 Disputa aberta! O suporte analisará a prova.");
    } else {
        if (!confirm("✅ CONFIRMAR APROVAÇÃO?\n\nO valor será debitado da sua reserva e enviado imediatamente para o colaborador.")) return;
        
        // Gil, esse status dispara o gatilho de pagamento automático que já soldamos
        await updateDoc(doc(db, "mission_submissions", docId), { 
