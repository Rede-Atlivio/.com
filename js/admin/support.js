...
export function init() {
    ...
    if(!container) return; // Proteção contra erro
    
    // Layout com botão de AÇÃO RÁPIDA
    container.innerHTML = `
        <div class="flex h-[calc(100vh-140px)] gap-4 animate-fade">
            <div class="w-1/3 glass-panel p-0 flex flex-col overflow-hidden border-r border-gray-700">
                <div class="p-4 bg-slate-900/50 border-b border-gray-700 flex justify-between items-center">
                    ...
                    <button onclick="window.marcarTudoLido()" class="text-[10px] bg-slate-700 text-slate-300 px-2 py-1 rounded hover:bg-slate-600 transition border border-slate-600">
                        ✅ Marcar tudo lido
                    </button>
                </div>
...
function carregarListaUsuarios() {
    ...
    unsubscribeList = onSnapshot(q, (snap) => {
        ...
        snap.forEach(d => {
            ...
            // Conta não lidos para o badge vermelho
...
window.abrirChatAdmin = async (uid, name, email) => {
    ...
    unsubscribeChat = onSnapshot(q, (snap) => {
        ...
        // Executa a limpeza no banco (Isso atualiza a Secretária)
        if (needsCommit) {
            batch.commit().then(() => console.log("🧹 Tickets lidos."));
        }
    });
};

// ============================================================================
// 4. AÇÕES (BOTÃO MÁGICO E ENVIO)
// ============================================================================
window.enviarRespostaAdmin = async (e) => {
    ...
    try {
        await addDoc(collection(window.db, "support_tickets"), {
            ...
            read: true // Resposta do admin já nasce lida
        });
...
window.marcarTudoLido = async () => {
    if(!confirm("Marcar todas as mensagens pendentes como lidas?")) return;
    
    const db = window.db;
    // Busca todas as mensagens não lidas enviadas por usuários
    const q = query(collection(db, "support_tickets"), where("read", "==", false), where("sender", "==", "user"));
    const snap = await getDocs(q);
    
    if(snap.empty) return alert("Nada pendente para limpar.");

    // Atualiza em lote (Batch)
    const batch = writeBatch(db);
    snap.forEach(d => batch.update(d.ref, { read: true }));
    
    await batch.commit();
    alert("✅ Tudo limpo! A secretária foi atualizada.");
};
