...
export async function init(viewType) {
    ...
    // EXPORTA FUNÇÕES GLOBAIS (CRUCIAL)
    ...
    // Atualiza botão de bulk action para abrir o modal de mensagem
            ...
            tbody.innerHTML += `
                <tr class="border-b border-white/5 hover:bg-white/5 transition group">
                    ...
                    <td class="p-3 text-right">
                        ...
                        <button onclick="window.openBalanceEditor('${data.id}', ${data.saldo||0}, '${data._displayName}')" class="bg-slate-700 hover:bg-emerald-600 text-white px-2 py-1 rounded text-xs font-bold" title="Financeiro">💰</button>
                    </td>
                </tr>`;
        } else {
...
async function openEditor(collectionName, id) {
    ...
    try {
        ...
        // LISTA DE SERVIÇOS (AQUI ESTAVA O PROBLEMA DOS BOTÕES)
        if (collectionName === 'active_providers' && id) {
             ...
             servicos.forEach((svc, idx) => {
                 ...
                 // Importante: Passamos os parametros como string segura
...
async function saveServiceAction(id, index, status) {
    ...
    try {
        ...
        if(services && services[index]) {
            ...
            // Se aprovou, manda notificação verde
...
function abrirModalMassa() {
    ...
    content.innerHTML = `
        <div class="space-y-4">
            ...
            <div>
                <label class="text-[10px] text-gray-400 font-bold uppercase">Mensagem (Notificação)</label>
                <textarea id="massa-msg" class="w-full p-3 rounded bg-white text-black" rows="3" placeholder="Ex: Parabéns! Você ganhou um bônus."></textarea>
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="text-[10px] text-gray-400 font-bold uppercase">Cor do Aviso</label>
                    <select id="massa-tipo" class="w-full p-3 rounded bg-white text-black">
                        <option value="success">🟢 Verde (Sucesso)</option>
                        <option value="alert">🟡 Amarelo (Atenção)</option>
                        <option value="error">🔴 Vermelho (Erro)</option>
                    </select>
                </div>
                <div>
                    <label class="text-[10px] text-gray-400 font-bold uppercase">Dar Crédito (R$)</label>
                    <input type="number" id="massa-credito" class="w-full p-3 rounded bg-white text-black" placeholder="0.00">
                </div>
            </div>

            <button onclick="window.enviarMassaConfirmado()" class="w-full bg-purple-600 hover:bg-purple-500 text-white py-4 rounded-xl font-bold uppercase shadow-lg">
                🚀 DISPARAR PARA TODOS
            </button>
        </div>
    `;
...
async function enviarMassaConfirmado() {
    ...
    if(!msg && credito === 0) return alert("Escreva uma mensagem ou dê um valor.");
    if(!confirm(`Confirmar envio para ${selectedUsers.size} usuários?`)) return;
    ...
    selectedUsers.forEach(uid => {
        ...
        // 2. Crédito (Atenção: Batch update não suporta increment fácil sem ler antes, faremos update simples)
        // Para segurança real em massa, idealmente seria cloud function, mas aqui faremos via loop assíncrono se tiver crédito.
    });

    await batch.commit();

    // Se tiver crédito, precisamos fazer um a um pois transactions não funcionam em batch grande
    if(credito !== 0) {
        for (let uid of selectedUsers) {
            try {
                const ref = doc(window.db, "usuarios", uid);
                // Pequena gambiarra para evitar ler tudo: update direto se existir saldo
                // Idealmente usar runTransaction, mas para massa, vamos assumir update
                const snap = await getDoc(ref);
                if(snap.exists()) {
                    const novoSaldo = (snap.data().saldo || 0) + credito;
...
async function executeUserBulkDelete() { 
    // Se clicou no ícone de raio (Ações em Massa)
    abrirModalMassa(); 
}
...
async function executeAdjustment(uid, val) {
    if(isNaN(val)) return;
    await updateDoc(doc(window.db, "usuarios", uid), { saldo: val, wallet_balance: val });
    alert("Saldo atualizado."); loadList();
}
