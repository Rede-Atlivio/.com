...
/**
 * MÓDULO: USERS V2 (INTEGRADO)
 * Gestão de Usuários, Prestadores, Saldos e Curadoria Individual.
 */
...
export async function init(viewType) {
    ...
    if (!headers) {
        console.error("❌ Elemento 'list-header' não encontrado no HTML.");
        return;
    }
    ...
    // --- EXPÕE FUNÇÕES GLOBAIS (NECESSÁRIO PARA ONCLICK NO HTML) ---
    window.openEditor = openEditor;
    window.saveAction = saveAction;
    window.saveServiceAction = saveServiceAction; // 🔥 Nova Função de Curadoria
...
async function loadList() {
    ...
    try {
        ...
        // Limite de 100 para performance
        ...
        if(countEl) countEl.innerText = `${allLoadedUsers.length} registros`;
        renderTable(allLoadedUsers);

    } catch (e) { 
        console.error("Erro loadList:", e);
        tbody.innerHTML = `<tr><td colspan="5" class="p-4 text-center text-red-400">Erro de conexão.</td></tr>`;
    }
}
...
function renderTable(lista) {
    ...
    lista.forEach(data => {
        ...
        if(currentType === 'users') {
                    ...
                    <td class="p-3">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <div class="text-[10px] text-gray-500 font-mono">${data.email || data.id.substring(0,8)+'...'}</div>
                            </div>
                        </div>
                    </td>
            ...
            const firstSvc = data.services?.[0]?.category || 'Sem serviço';
            const extraSvc = data.services?.length > 1 ? `<span class="text-[9px] ml-1 bg-slate-800 px-1 rounded">+${data.services.length-1}</span>` : '';
                ...
                <tr class="border-b border-white/5 hover:bg-white/5 transition group ${rowOpacity}">
                    ...
                    <td class="p-3">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <div class="font-bold text-white text-sm">${data._displayName}</div>
                                <div class="text-[10px] text-gray-500">${data.bio ? data.bio.substring(0,25)+'...' : 'Sem bio'}</div>
                            </div>
                        </div>
                    </td>
                    <td class="p-3 text-gray-400 text-xs">
                        <span class="bg-slate-800 px-2 py-1 rounded border border-slate-700">${firstSvc}</span>${extraSvc}
                    </td>
                    <td class="p-3 text-xs font-bold text-white flex items-center gap-2 mt-2">
                        <span>${statusIcon}</span> ${statusLabel.toUpperCase()}
                    </td>
                    <td class="p-3 text-right opacity-80 group-hover:opacity-100 transition">
                        <button onclick="window.openEditor('active_providers','${data.id}')" class="bg-blue-600 hover:bg-blue-500 text-white px-4 py-1.5 rounded text-xs font-bold shadow transition transform active:scale-95">CURADORIA</button>
                    </td>
                </tr>`;
        }
    });

    // Reativa listeners dos checkboxes individuais
...
async function openEditor(collectionName, id) {
    ...
    try {
        ...
        // BLOCO 1: MÍDIA (Foto e Banner)
        if (data.banner_url || data.foto_perfil) {
            html += `<div class="bg-slate-800 p-4 rounded-xl border border-slate-700 mb-4 flex gap-4">`;
            if(data.banner_url) html += `<div><p class="text-[9px] text-gray-500 mb-1">BANNER</p><img src="${data.banner_url}" class="h-20 w-32 object-cover rounded border border-gray-600"></div>`;
            if(data.foto_perfil) html += `<div><p class="text-[9px] text-gray-500 mb-1">AVATAR</p><img src="${data.foto_perfil}" class="w-16 h-16 object-cover rounded-full border border-gray-600"></div>`;
            html += `</div>`;
        }

        // BLOCO 2: CURADORIA INDIVIDUAL (Lógica Nova)
        ...
        // BLOCO 3: FINANCEIRO (Apenas para visão de Usuários)
        ...
        // BLOCO 4: CAMPOS GERAIS (Editor Automático)
        const keys = id ? Object.keys(data).sort() : ['nome', 'email', 'tipo', 'status']; 
        // Campos que não devemos editar manualmente
        const ignored = ['created_at', 'updated_at', 'services', 'geo_location', 'is_demo', 'visibility_score', 'saldo', '_displayName', 'id', 'push_token'];
        ...
        // BLOCO 5: BOTÕES DE AÇÃO GLOBAL
        html += `
            <div class="border-t border-slate-700 pt-6 mt-6">
                <p class="text-center text-gray-500 text-[9px] uppercase font-bold mb-3">⚠️ Ações da Conta (Afeta Tudo)</p>
                <div class="flex gap-3">
                    <button onclick="window.saveAction('${realCollection}', '${id}', 'banir')" class="flex-1 bg-red-600 hover:bg-red-500 text-white py-3 rounded-lg font-bold text-xs shadow-lg">⛔ BANIR CONTA</button>
                    <button onclick="window.saveAction('${realCollection}', '${id}', 'salvar')" class="flex-1 bg-blue-600 hover:bg-blue-500 text-white py-3 rounded-lg font-bold text-xs shadow-lg">💾 SALVAR DADOS GERAIS</button>
                </div>
                <div class="mt-2 text-center">
                    <p class="text-[9px] text-gray-600">Para aprovar serviços, use os botões na lista de serviços acima.</p>
                </div>
            </div>`;
        ...
        console.error(e);
        content.innerHTML = `<p class="text-red-500 text-center p-4">Erro ao carregar dados: ${e.message}</p>`; 
    }
}
...
async function saveServiceAction(id, index, status) {
    ...
    try {
        ...
        // Envia notificação
        let msg = status === 'aprovado' ? `Seu serviço de ${services[index].category} foi APROVADO! 🟢` : `Seu serviço de ${services[index].category} precisa de atenção. Verifique o status. 🔴`;
        await addDoc(collection(db, "notifications"), { 
            uid: id, 
            message: msg, 
            read: false, 
            created_at: serverTimestamp(), 
            type: status === 'aprovado' ? 'success' : 'alert' 
        });

        alert("✅ Status do serviço atualizado!");
        window.openEditor('active_providers', id); // Recarrega para ver a mudança
...
function updateUserBulkUI() { 
    ...
    if(selectedUsers.size > 0) { 
        ...
        if(count) count.innerText = selectedUsers.size; 
    } else { 
...
function openBalanceEditor(uid, currentBalance, nomeUser) { 
    const modal = document.getElementById('modal-editor'); 
    const content = document.getElementById('modal-content'); 
    const title = document.getElementById('modal-title'); 
    ...
    modal.classList.remove('hidden'); 
    if(title) title.innerText = "FINANCEIRO"; 
    ...
    content.innerHTML = `
        <div class="p-4 bg-slate-800 rounded-xl border border-slate-700 mb-6 text-center animate-fade">
            ...
            <p class="text-xs text-gray-500 mt-2">${nomeUser}</p>
        </div>
        <div class="grid grid-cols-2 gap-4 animate-fade">
            <button onclick="window.setTransactionMode('credit')" id="btn-mode-credit" class="bg-emerald-900/50 border border-emerald-500/30 text-white p-4 rounded-xl hover:bg-emerald-900/80 transition"><p class="font-bold text-emerald-400">🟢 CRÉDITO</p></button>
            <button onclick="window.setTransactionMode('debit')" id="btn-mode-debit" class="bg-red-900/50 border border-red-500/30 text-white p-4 rounded-xl hover:bg-red-900/80 transition"><p class="font-bold text-red-400">🔴 DÉBITO</p></button>
        </div>
        <div id="trans-form" class="mt-6 hidden animate-fade">
            ...
            <input type="text" id="trans-desc" class="inp-editor mb-4" placeholder="Motivo do ajuste">
            <button onclick="window.executeAdjustment('${uid}')" class="w-full bg-blue-600 hover:bg-blue-500 text-white py-3 rounded-lg font-bold text-xs uppercase shadow-lg">CONFIRMAR</button>
        </div>`; 
...
async function executeAdjustment(uid) { 
    const amount = parseFloat(document.getElementById('trans-amount').value); 
    const desc = document.getElementById('trans-desc').value; 
    const mode = window.tempTransMode; 
    
    if (!amount || amount <= 0) return alert("Digite valor válido."); 
    if (!desc) return alert("Digite o motivo do ajuste."); 
    
    const finalAmount = mode === 'credit' ? amount : -amount; 
    if(!confirm(`Confirma ajuste de R$ ${finalAmount} para este usuário?`)) return; 
    ...
    try { 
        ...
        await runTransaction(db, async (transaction) => { 
            ...
            // Opcional: Registrar no histórico (se houver coleção transactions)
            /* const transRef = doc(collection(db, "transacoes"));
            transaction.set(transRef, { uid, valor: finalAmount, motivo: desc, data: serverTimestamp() });
            */
        }); 
