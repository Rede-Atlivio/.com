...
// ============================================================================
// 1. CARREGAR CARTEIRA (CONSTRUTOR DE INTERFACE)
// ============================================================================
export async function carregarCarteira() {
    console.log("💰 Iniciando módulo Carteira (Injeção Visual)...");
    const uid = auth.currentUser?.uid;
    if(!uid) return;

    // Tenta encontrar a section da carteira. 
    // Se o seu HTML usa 'sec-ganhar' ou 'sec-carteira', o código abaixo acha.
    let container = document.getElementById('sec-carteira');
    if(!container) container = document.getElementById('sec-ganhar'); // Fallback

    if (!container) return console.error("❌ Erro: Não achei a div da carteira no HTML.");

    // 🏗️ INJEÇÃO DE HTML (CRIA O PAINEL NOVO)
    // Isso apaga o que tinha antes e cria a estrutura de Saldo Livre vs Reservado
    container.innerHTML = `
...
export function atualizarCarteira() {
    ...
    if(userProfile) {
        ...
        const saldoPreso = parseFloat(userProfile.wallet_reserved || 0); // Novo campo

        // Atualiza Disponível
        if(elAvailable) {
            elAvailable.innerText = `R$ ${saldoLivre.toFixed(2).replace('.', ',')}`;
            // Cor muda se estiver negativo
        ...
        // Atualiza Header do Topo (Miniatura)
        const headerSaldo = document.querySelector('#header-user-wallet span'); 
        // Nota: Ajuste o seletor acima conforme seu HTML do header, se existir
...
// ============================================================================
// 3. FUNÇÕES DE CUSTÓDIA (O COFRE)
// ============================================================================

// A: Reservar Saldo (Cliente Confirma -> Dinheiro Bloqueado)
...
// B: Liberar Saldo (Serviço Concluído -> Dinheiro vai pro Prestador)
// Nota: Essa função move do "Reservado Cliente" -> "Carteira Prestador"
// Mas por segurança, vamos criar primeiro a função que zera a reserva do cliente.
...
// C: Estornar Saldo (Cancelamento -> Dinheiro volta pro Disponível)
export async function estornarSaldo(uid, valor) {
    ...
    await runTransaction(db, async (t) => {
        ...
        t.update(userRef, {
            wallet_balance: atualLivre + valor,
            wallet_reserved: (atualPreso - valor < 0 ? 0 : atualPreso - valor)
        });
    });
    // Atualiza localmente
    if(window.userProfile) {
...
export async function processarCobrancaTaxa(orderId, valorServico) {
    // Mantém compatibilidade, mas agora debita do saldo livre
    if (!auth.currentUser) return;
    const uid = auth.currentUser.uid;
    const valorTaxa = valorServico * TAXA_PLATAFORMA;
    
    // Simples decremento (O fluxo novo usará a função de custódia no futuro)
    const userRef = doc(db, "usuarios", uid);
    await runTransaction(db, async (t) => {
        const docSnap = await t.get(userRef);
